...
fillInNAs <- function(steps,date) {
   if (!is.na(steps)) {
       ...
       meanPerDay[as.character(date)]
   }
}

filledInNAs <- data.frame(date=data$date,interval=data$interval,steps=mapply(fillInNAs, data$steps, data$date))
```

Now let's see how a histogram of steps per day compares 
