...
That means that all the `NA`s are in the `steps` column.  Let's fill
in each `NA` with the mean number of steps from the given day.

```{r}
fillInNAs <- function(steps,date) {
   if (!is.na(steps)) {
       steps
   } else {
       stepsPerDay[as.character(date)]
   }
}
data$steps <- mapply(fillInNAs, data$steps, data$date)
```
